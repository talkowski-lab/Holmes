...
#Run preferences
export COHORT_ID=your_cohort_id_here #cohort ID (string)
export OUTDIR=your_output_directory_here #output directory - only final outputs will be written here
export WRKDIR=${OUTDIR}/TMPFILES #working directory - all temp files will be written here. May temporarily need lots of storage, so best to avoid writing to /tmp
export other_assign=FEMALE #sex group to assign "other" sex for depth-based calling. must be either "MALE" or "FEMALE"
...
export DNAcopy_ref=/data/talkowski/rlc47/src/DNAcopy_reference.bindata.bed #DNAcopy reference profile - MALE
...
export bamstat_paths=${OUTDIR}/bamstat.list #file containing paths to pre-run bamstat directories. First column: ID, second column: full path to bamstat directory; tab-delimited. Will be ignored unless pre_bamstat="TRUE"
