...
if [ $# -eq 6 ]; then
  ...
  module load /apps/modulefiles/lab/miket/sambamba/0.4.7
  ...
  if [ ${mode} == "physical" ]; then
    ...
    while read bam sample; do
      ...
      while read contig_line; do
        ...
        bsub -sla miket_sc -R 'rusage[mem=4000]' -M 4000 -v 6000 -q short -e ${OUTDIR}/logfiles/error.txt -o ${OUTDIR}/logfiles/splitPropPairs.out -J ${ID}_step1 "sambamba view -f bam -F 'paired and proper_pair and not (unmapped or mate_is_unmapped or duplicate or secondary_alignment)' ${bam} ${contig} | /apps/lab/miket/samtools/1.0/bin/samtools sort -T ${TMPDIR}/${sample}.${contig}.screened -O bam -m 3200M /dev/stdin > ${TMPDIR}/${sample}/${sample}.${contig}.screened.bam"
  ...
  if [ ${mode} == "nucleotide" ]; then
    ...
    while read contig length; do
      while read bam sample; do
        bsub -q normal -J ${ID}_step1 "sambamba view -f bam -F 'not secondary_alignment and not duplicate' ${bam} ${contig} | bedtools coverage -counts -abam - -b ${TMPDIR}/${contig}.bins | sort -nk2,2 > ${TMPDIR}/${sample}.${contig}.coverage.bed"
      done < ${list}
    done < ${TMPDIR}/contig.list
  fi
  ...
  if [ ${mode} == "physical" ]; then
    if [ $num_contigs -gt 1 ]; then 
     ...
     while read bam sample; do
      bsub -sla miket_sc -q big -R 'rusage[mem=16000]' -e ${OUTDIR}/logfiles/error.txt -o ${OUTDIR}/logfiles/mergeAlignments.out -M 16000 -v 24000 -J ${ID}_mergefiltered "/data/talkowski/tools/bin/sambamba merge -l 6 ${TMPDIR}/${sample}/${sample}.merged.bam ${TMPDIR}/${sample}/${sample}.*.screened.bam; /data/talkowski/tools/bin/sambamba sort -n --tmpdir=${TMPDIR} -m 12GB -l 6 -o ${TMPDIR}/${sample}.screened.nsort.bam ${TMPDIR}/${sample}/${sample}.merged.bam"
     done < ${list}
    elif [ $num_contigs -eq 1 ]; then
     echo -e "\nSORTING FILTERED ALIGNMENTS...\n"
     contig=$( awk '{ print $1 }' ${TMPDIR}/contig.list )
     while read bam sample; do
      bsub -sla miket_sc -q big -R 'rusage[mem=16000]' -e ${OUTDIR}/logfiles/error.txt -o ${OUTDIR}/logfiles/mergeAlignments.out -M 16000 -v 24000 -J ${ID}_mergefiltered "/data/talkowski/tools/bin/sambamba sort -m 4GB -n -l 6 -o ${TMPDIR}/${sample}.screened.nsort.bam ${TMPDIR}/${sample}/${sample}.${contig}.screened.bam"
     done < ${list}
    fi
    ##Gate (20 second check; 5 minute report)
    echo -e "\n"
  ...
  if [ ${mode} == "physical" ]; then
    ...
    while read bam sample; do
      bsub -sla miket_sc -e ${OUTDIR}/logfiles/error.txt -o ${OUTDIR}/logfiles/fragmentSynthesis.out -q normal -J ${ID}_synthesizefragments "bedtools bamtobed -bedpe -i ${TMPDIR}/${sample}.screened.nsort.bam | awk -v OFS=\"\t\" '{ if (\$2>=0 && \$6>\$2) print \$1, \$2, \$6 }' > ${TMPDIR}/${sample}.fragments.bed"
    done < ${list}
    ##Gate (20 second check; 5 minute report)
    echo -e "\n"
