...
######################################
# Talkowski Lab
# Consensus CNV Variant Merger
######################################

#Read input
in_list=$1             #input list. Format: sample ID <TAB> full/path/to/bedfile/to/be/merged
cluster_dist=$2        #maximum distance between breakpoints for concordance, in bases. 10kb for Eco
...
#9) sample info (bracket-enclosed, slash-delimmed within samples, pipe-delimmed between samples)
...
while read chr start end ID eID info; do
  ...
  if [ $( fgrep -w ${eID} ${remove} | wc -l ) -eq 0 ]; then
    ...
    if [ $( cat ${clust} | wc -l ) -gt 1 ]; then
      ...
      cat ${clust} | wc -l
      cut -f4 ${clust} | paste -s -d, | sed -e 's/^/[/g' -e 's/$/]/g'
      cut -f6 ${clust} | paste -s -d\| | sed -e 's/^/[/g' -e 's/$/]/g'
      cut -f5 ${clust} >> ${remove}
    else
