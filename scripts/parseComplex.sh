...
OUTDIR=$6              #output directory.  Will write to ${OUTDIR}/transloc.classifications.list
cdist=$7               #clustering distance used in cohort
n_cohort=$8            #number of samples in cohort; used to flag artifacts

#TEST PARAMETERS - SFARI ECO
linker=/scratch/miket/rlc47temp/FilteredReadsBank/classifier_reruns/SFARIEco/clusterfix/Complex/ComplexEvents.txt
events=/scratch/miket/rlc47temp/FilteredReadsBank/classifier_reruns/SFARIEco/clusterfix/newCoords/events.list
clusters=/scratch/miket/rlc47temp/FilteredReadsBank/classifier_reruns/SFARIEco/clusterfix/newCoords/clusters.list
cnMOPS_dels=/data/talkowski/Samples/SFARI/miscFiles/cnMOPS.del.Eco.list
cnMOPS_dups=/data/talkowski/Samples/SFARI/miscFiles/cnMOPS.dup.Eco.list
OUTDIR=/data/talkowski/Samples/SFARI/Newest_Eco_Classifier/clusterfix/newCoords
cdist=9600
n_cohort=238
...
while read cxID oIDs; do
  ...
  #Remapping protocol if any mq < 9.25 (equivalent to > 3/4 reads in cluster multiply mapped), at most one single mq=0
    ...
    #remap del- if lowest
    if [ ${lowmq} == "delm" ]; then
      #create blat interval ± 6 clustering distances out on either side
...
done < <( cut -f7 ${preflag} ) > ${OUTDIR}/complex_classifications.list
...
done < ${POOL} >> ${OUTDIR}/complex_classifications.list
...
while read cxID oIDs; do
  ...
  #Otherwise attempt to classify
    ...
    #If +/+ and -/- clusters straddle del cluster, then small inversion w/flanking del (limited to flanking del ≤ 3*clustering distance)
...
while read cxID; do
  if [ $( fgrep -w ${cxID} ${linker} | cut --complement -f1 | sed 's/\t/\n/g' | fgrep deletion | wc -l ) -eq 2 ]; then
    if [ $( fgrep -w ${cxID} ${linker} | cut --complement -f1 | sed 's/\t/\n/g' | fgrep insertion | wc -l ) -eq 1 ]; then
      fgrep  -w ${cxID} ${linker} >> ${POOL}
    fi
  fi
done < <( awk -v OFS="\t" '{ if (NF==4) print $0 }' ${linker} | fgrep deletion | fgrep insertion | cut -f1 )
