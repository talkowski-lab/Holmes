...
DEL)
  while read chr start end class eID; do
    ...
    ${liWGS_SV}/scripts/build_exons.sh hg19 "chr${chr}" ${start} ${end} \
    ${TMPDIR}/${ID}.${chr}_${start}_${end}.buildExons.tmp
    if [ -e ${TMPDIR}/${ID}.${chr}_${start}_${end}.buildExons.tmp ]; then
    genes=$( bedtools intersect -wb -a <( echo -e "${chr}\t${start}\t${end}" ) \
      -b <( sed 's/^chr//g' ${TMPDIR}/${ID}.${chr}_${start}_${end}.buildExons.tmp ) | \
...
DUP)
  while read chr start end class eID; do
    ...
    -b ${refFlat} | cut -f7 | sort | uniq > ${TMPDIR}/b1.tmp
    bedtools intersect -wb -a <( echo -e "${chr}\t${end}\t$( echo "${end}+1" | bc )" ) \
    -b ${refFlat} | cut -f7 | sort | uniq > ${TMPDIR}/b2.tmp
    ...
    if [ $( cat ${TMPDIR}/ovr.tmp | wc -l ) -gt 0 ]; then
      ...
      if [ -e ${TMPDIR}/${ID}.${chr}_${start}_${end}.buildExons.tmp ]; then
        LOF=$( bedtools intersect -wb -a <( echo -e "${chr}\t${start}\t${end}" ) \
          -b <( sed 's/^chr//g' ${TMPDIR}/${ID}.${chr}_${start}_${end}.buildExons.tmp ) | \
          awk '{ print $NF }' | sort | uniq | fgrep -wf ${TMPDIR}/ovr.tmp | \
...
INS_SOURCE)
  while read chr start end class eID; do
    ...
    bedtools intersect -wb -a <( echo -e "${chr}\t${end}\t$( echo "${end}+1" | bc )" ) \
    -b ${refFlat} | cut -f7 | sort | uniq > ${TMPDIR}/b2.tmp
    LOFa=$( echo "$( fgrep -wvf ${TMPDIR}/b1.tmp ${TMPDIR}/b2.tmp | awk -v ORS="," \
      '{ print "pLOF_"$1 }' )$( fgrep -wvf ${TMPDIR}/b2.tmp ${TMPDIR}/b1.tmp | \
...
INV)
  while read chr start end class eID; do
    ...
    bedtools intersect -wb -a <( echo -e "${chr}\t${end}\t$( echo "${end}+1" | bc )" ) \
    -b ${refFlat} | cut -f7 | sort | uniq > ${TMPDIR}/b2.tmp
    LOFa=$( echo "$( fgrep -wvf ${TMPDIR}/b1.tmp ${TMPDIR}/b2.tmp | \
      awk -v ORS="," '{ print "LOF_"$1 }' )$( fgrep -wvf ${TMPDIR}/b2.tmp ${TMPDIR}/b1.tmp | \
      awk -v ORS="," '{ print "LOF_"$1 }' )" )
    fgrep -wf ${TMPDIR}/b1.tmp ${TMPDIR}/b2.tmp > ${TMPDIR}/ovr.tmp
    if [ $( cat ${TMPDIR}/ovr.tmp | wc -l ) -gt 0 ]; then
      if [ -e ${TMPDIR}/${ID}.${chr}_${start}_${end}.buildExons.tmp ]; then
      ...
      if [ -e ${TMPDIR}/${ID}.${chr}_${start}_${end}.buildExons.tmp ]; then
        LOFb=$( bedtools intersect -wb -a <( echo -e "${chr}\t${start}\t${end}" ) \
          -b <( sed 's/^chr//g' ${TMPDIR}/${ID}.${chr}_${start}_${end}.buildExons.tmp ) | \
          awk '{ print $NF }' | sort | uniq | fgrep -wf ${TMPDIR}/ovr.tmp | awk -v ORS="," '{ print "LOF_"$1 }' )
      else
        LOFb=""
      fi
    else
