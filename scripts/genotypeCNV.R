...
#
# Joseph Glessner, Harrison Brand, and Ryan Collins
# Update October 2015 (load sample set prior)
# Update August 2015 (for incorporation into Holmes liWGS-SV 1.0)
# Update November 2015 (shift to power test & permuted p-value for all sites)
#---------------------------
...
genotypeCov <- function(chr,start,end,            #region to be genotyped
  ...
  if((start < CentromereEndsHg19[as.numeric(chrIndex)]) & (end > CentromereStartsHg19[as.numeric(chrIndex)])){
  ...
  if(a$freq[1]>1){
    ...
    ClustSolStats="NA";ClustSolStats$n.between="NA"} #only 1 cluster so cluster.stats does not work
  ...
  if(nrow(kmean_matrix)>1){ 
    ...
    if(prior==TRUE && length(kmean_matrix[which(genotype[,5:ncol(genotype)]==(ifelse(delPrior=="TRUE",1,3)))])>1 && length(kmean_matrix[which(genotype[,5:ncol(genotype)]==2)])>1 && !anyNA(cor(kmean_matrix))){
      ...
      if(ncol(kmean_matrix)==2){a<-1;b<-2}else{a<-2;b<-ncol(kmean_matrix-1)}
      for(i in a:b){ 
...
results_matrix <- as.data.frame(t(apply(intervals,1,function(row){return(suppressWarnings(genotypeCov(chr=row[1],
                                                                                                      ...
                                                                                                      ID=row[4], verbose=args[4], outType=args[5], IDs=row[5], prior=args[6], delPrior=args[7])))})))
