...
int getReadPairGroup (ifstream &f, long &startLoc, list<readPair* > ** dL, long DIST_CUTOFF) {
    ...
    while (!f.eof()) {
        ...
        //cout << "current position: " << f.tellg() << "\t" <<  line << "\n";
        ...
        if (v->size() > 0) {
            ...
            if (prev == NULL || !(prev->chrA == rp->chrA && prev->chrB == rp->chrB && prev->posA == rp->posA && prev->posB == rp->posB && prev->readID == rp->readID)) {
...
void printUsage() {
  cerr << "usage:  readPairCluster -r readpairfile.txt [-d DIST_CUTOFF] [-q MAPQ_THRESH] [-s MIN_CLUST_SIZE] [-h]" << endl;
  cerr << endl;
  cerr << "options: " << endl;
  cerr << "  -d DISTANCE_CUTOFF    Max distance between ends of reads that are clustered together [3000]" << endl;
  cerr << "  -q MAPQ_THRESHOLD     Quality score cutoff for including reads [50]" << endl;
  cerr << "  -s MIN_CLUST_SIZE     Minimum size of clusters to output [3]" << endl;
    ...
    while (first != datalist->end()) {
        ...
        if (distance(first, last) > 0) {
            ...
            for (r = first; r != last; r++) {
                ...
                if (rprev == r || (*r)->readID != (*rprev)->readID ) {
                    sprev = r;
                    for ( s = r; s != next; s++) {
...
int main(int argc, char **argv) {
  ...
  if (! f.good()) {
    f.close();
    cerr << "Read pair file does not exist" << endl;
    exit(1);
  }
