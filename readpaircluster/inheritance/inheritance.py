...
def read_sv(bed, size=500, freq=0.9, samples=None, bedpe=True):
    """
    Parameters
    ----------
    bed : str or handle
    ...
    # Size filter
    sv = sv[(sv['end'] - sv['start']) >= size].copy()
    ...
    # bed = sv.loc[:, ['#chrA', 'start', 'end']].copy()
...
def compute_freq(sv):
    ...
    def is_parent(sample):
        if 'SFARI_d' not in sample:
            return 'not_sample'
        if 'fa' in sample or 'mo' in sample:
            return 'parent'
        else:
            return 'child'

    sv = sv.astype(bool).astype(int)
    frequency = sv.groupby(member, axis=1).sum()
    frequency = frequency.drop('not_sample', axis=1)

    frequency['total'] = frequency.sum(axis=1)
    frequency['parent'] = frequency['mo'] + frequency['fa']
...
def compute_inheritance(sv):
    ...
    inherit = sv.groupby(lambda s: s[:-2], axis=1) \
        .apply(lambda g: g.apply(status, axis=1))

    melted = pd.melt(inherit, var_name='sample', value_name='inherit')
    counts = pd.crosstab(index=[melted['inherit']], columns=[melted['sample']])

    return inherit, counts.transpose()
